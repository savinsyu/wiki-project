...
@bp.route("/wiki/download-pdf")
def wiki_download_pdf():
    conn = connect.get_db_connection()
    wiki_list = conn.execute(
        "SELECT wiki_name, wiki_content FROM wiki ORDER BY wiki_id DESC"
    ).fetchall()
    conn.close()
    ...
    return send_file(
        buffer,
        as_attachment=True,
        download_name="wiki_commands.pdf",
